...
pixels <- uniquepts[, {
	...
	if (nrow(DT) > 0) {
		...
		DT[, q := quantile(meanIRG, probs = 0.8, na.rm = TRUE)]
...
morans <- blockids[, {
	...
	if (out[['npixels']] == 0) {
		...
		# Observed
		locs[blockidyr == biy,
				 good := extract(r, matrix(c(EASTING, NORTHING), ncol = 2))]
...
### Calculate time in patch ----
# Merge morans onto observed locs
locs[morans,
		 c('moran', 'quantile', 'error', 'propGood', 'npixels') :=
		 	.(moran, quantile, error, propGood, npixels),
		 on = 'blockidyr']


# Time spent in patch
locs[, numbNA := sum(is.na(good)), blockidyr]
locs[, numbNonNA := .N - numbNA, blockidyr]
locs[, tInPatch := sum(good, na.rm = TRUE) / numbNonNA, blockidyr]

# Output patchiness and morans
patchiness <-
	unique(locs[, .(blockidyr,
									numbNonNA,
									numbNA,
									error,
									moran,
...
### Output ----
saveRDS(locs, paste0(derived, 'observedPatch.Rds'))
saveRDS(patchiness, paste0(derived, 'patchiness.Rds'))
